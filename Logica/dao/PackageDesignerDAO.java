...
public class PackageDesignerDAO implements PackageDesignerDAOInterface {
	...
	public int insert(PackageDesigner packageDesigner) throws SQLException {
		...
		cs.setString(1, packageDesigner.getUserName());
		cs.setString(2, packageDesigner.getPassword());
		cs.setInt(3, packageDesigner.getRolId());
	...
	public void update(PackageDesigner packageDesigner) throws SQLException {
		...
		cs.setDate(5, Date.valueOf(packageDesigner.getStartDateConnection()));
		cs.setDate(6, Date.valueOf(packageDesigner.getLastDateConnection()));
		cs.setBoolean(7, packageDesigner.isConnected());
		cs.execute(); // se ejecuta la llamada a la funcion
		cs.close(); // se cierra la llamada a la funcion
	}
	...
	public PackageDesigner select(int idPackageDesigner) throws SQLException {
		...
		PackageDesigner packageDesigner = mapEntity(cs); // se mapea la entidad a objeto
		cs.close(); // se cierra la llamada a la funcion
