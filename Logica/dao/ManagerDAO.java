...
public class ManagerDAO implements ManagerDAOInterface {
	...
	public int insert(Manager manager) throws SQLException {
		...
		cs.setString(1, manager.getUserName());
		cs.setString(2, manager.getPassword());
		cs.setInt(3, manager.getRolId());
	...
	public void update(Manager manager) throws SQLException {
		...
		cs.setDate(5, Date.valueOf(manager.getStartDateConnection()));
		cs.setDate(6, Date.valueOf(manager.getLastDateConnection()));
		cs.setBoolean(7, manager.isConnected());
		cs.execute(); // se ejecuta la llamada a la funcion
		cs.close(); // se cierra la llamada a la funcion
	}
	...
	public Manager select(int idManager) throws SQLException {
		...
		Manager manager = mapEntity(cs); // se mapea la entidad a objeto
		cs.close(); // se cierra la llamada a la funcion
