...
public class AccommodationModalityDAO implements AccommodationModalityDAOInterface {
    ...
    public void delete(int idAccommodationModality) throws SQLException {
        ...
        this.cache.remove(idAccommodationModality); // Se elimina la modalidad de cache
    }
    ...
    public AccommodationModality mapEntity(CallableStatement cs) throws SQLException { // Metodo encargado del mapeo (Explicacion del mapeo en la documentación del proyecto)
        ...
        if (accommodationModality == null) {
            ...
            accommodationModality.actualizarCampos(this.accommodationContract, cs.getResultSet().getString("type_of_modality"), TypeOfRoomDAO.getInstancie().select(cs.getResultSet().getInt("type_of_room_id")),
                    MealPlanDAO.getInstancie().select(cs.getResultSet().getInt("meal_plan_id")), HotelModalityDAO.getInstancie().select(cs.getResultSet().getInt("hotel_modality_id")),
                    cs.getResultSet().getInt("cant_days_accommodation"), cs.getResultSet().getDouble("price")); // se actualizan los campos de la modalidad guardada en cache
